...
describe('ToyRobotComponent', () => {
  ...
  it('should add command to command history', () => {
    let command = "Some string";
    component.command = command;
    component.onCommand();
    let findCommand = component.commandHistory.filter(x =>  x.command === command);
  ...
  testValidMoveCommands.forEach(test => {
    it('should move with valid MOVE command', () => {
  ...
  testInvalidMoveCommands.forEach(test => {
    it('should not move with invalid MOVE command', () => {
      component.robotPosition = test.position;
      component.executeCommand("MOVE");
  ...
  testDirectionCases.forEach(test => {
    it('should change direction for LEFT or RIGHT commands', () => {
  ...
  it('should print current robot position on REPORT command', () => {
    ...
    let report = `X: ${component.robotPosition.x+1}, Y: ${component.robotPosition.y+1}, Direction: ${component.robotPosition.direction}`;
    let findCommand = component.commandHistory.filter(x =>  x.command === report);
    expect(findCommand.length > 0).toBeTruthy();
  });

  const testOtherCommands = ["LEFT", "MOVE", "REPORT"];
  testOtherCommands.forEach((cmd) => {
    it('should not allow other commands until valid PLACE command made', () => {
  ...
  testInvalidPlaceCommands.forEach(test => {
    it('should not place robot on invalid PLACE command', () => {
      component.isValidCommand(test);
